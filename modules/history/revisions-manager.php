...
use Embroidery\Post_CSS_File;
use Embroidery\Utils;
...
class Revisions_Manager {
	...
	public static function get_revisions( $post_id = 0, $query_args = [], $parse_result = true ) {
		...
		foreach ( $posts as $revision ) {
			$date = date_i18n( _x( 'M j @ H:i', 'revision date format', 'embroidery' ), strtotime( $revision->post_modified ) );
	...
	public static function update_autosave( $autosave_data ) {
		...
		Plugin::$instance->db->safe_copy_embroidery_meta( $autosave_data['post_parent'], $revision_id );
	}
	...
	public static function save_revision( $revision_id ) {
		...
		if ( $parent_id ) {
			Plugin::$instance->db->safe_copy_embroidery_meta( $parent_id, $revision_id );
		}
	}

	public static function restore_revision( $parent_id, $revision_id ) {
		$is_built_with_embroidery = Plugin::$instance->db->is_built_with_embroidery( $revision_id );

		Plugin::$instance->db->set_is_embroidery_page( $parent_id, $is_built_with_embroidery );

		if ( ! $is_built_with_embroidery ) {
			return;
		}

		Plugin::$instance->db->copy_embroidery_meta( $revision_id, $parent_id );
	...
	public static function on_revision_data_request() {
		...
		if ( ! current_user_can( 'edit_post', $revision->ID ) ) {
			wp_send_json_error( __( 'Access Denied.', 'embroidery' ) );
		}
	...
	public static function on_delete_revision_request() {
		...
		if ( $deleted && ! is_wp_error( $deleted ) ) {
			...
			wp_send_json_error( __( 'Cannot delete this Revision', 'embroidery' ) );
		}
	}

	public static function add_revision_support_for_all_post_types() {
		$post_types = get_post_types_by_support( 'embroidery' );
	...
	public static function editor_settings( $settings, $post_id ) {
		$settings = array_replace_recursive( $settings, [
			...
			'current_revision_id' => self::current_revision_id( $post_id ),
			'i18n' => [
				'edit_draft' => __( 'Edit Draft', 'embroidery' ),
				'edit_published' => __( 'Edit Published', 'embroidery' ),
				'no_revisions_1' => __( 'Revision history lets you save your previous versions of your work, and restore them any time.', 'embroidery' ),
				'no_revisions_2' => __( 'Start designing your page and you\'ll be able to see the entire revision history here.', 'embroidery' ),
				'current' => __( 'Current Version', 'embroidery' ),
				'restore' => __( 'Restore', 'embroidery' ),
				'restore_auto_saved_data' => __( 'Restore Auto Saved Data', 'embroidery' ),
				'restore_auto_saved_data_message' => __( 'There is an autosave of this post that is more recent than the version below. You can restore the saved data fron the Revisions panel', 'embroidery' ),
				'revision' => __( 'Revision', 'embroidery' ),
				'revision_history' => __( 'Revision History', 'embroidery' ),
				'revisions_disabled_1' => __( 'It looks like the post revision feature is unavailable in your website.', 'embroidery' ),
				'revisions_disabled_2' => sprintf(
					/* translators: %s: Codex URL */
					__( 'Learn more about <a targe="_blank" href="%s">WordPress revisions</a>', 'embroidery' ),
	...
	private static function register_actions() {
		...
		add_filter( 'embroidery/editor/localize_settings', [ __CLASS__, 'editor_settings' ], 10, 2 );
		add_filter( 'embroidery/ajax_save_builder/return_data', [ __CLASS__, 'ajax_save_builder_data' ] );
		add_action( 'embroidery/db/before_save', [ __CLASS__, 'db_before_save' ], 10, 2 );
		add_action( '_wp_put_post_revision', [ __CLASS__, 'save_revision' ] );
		add_action( 'wp_creating_autosave', [ __CLASS__, 'update_autosave' ] );

		if ( Utils::is_ajax() ) {
			add_action( 'wp_ajax_embroidery_get_revision_data', [ __CLASS__, 'on_revision_data_request' ] );
			add_action( 'wp_ajax_embroidery_delete_revision', [ __CLASS__, 'on_delete_revision_request' ] );
		}
	}
