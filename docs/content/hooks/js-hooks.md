...
```javascript
jQuery( function( $ ) {
	// Add space for Embroidery Menu Anchor link
	if ( undefined !== window.embroideryFrontend ) {
		embroideryFrontend.hooks.addFilter( 'frontend/handlers/menu_anchor/scroll_top_distance', function( scrollTop ) {
			return scrollTop - 30;
		} );
	}
} );
...
#### Example
 ...
 ```javascript
embroideryFrontend.hooks.addAction( 'init', function() {
  // Do something that is based on the embroideryFrontend object.
} );
 ```
...
```javascript
embroideryFrontend.hooks.addAction( 'frontend/element_ready/global', function( $scope ) {
	if ( $scope.data( 'shake' ) ){
		$scope.shake();
	}
} );
...
```javascript
embroideryFrontend.hooks.addAction( 'frontend/element_ready/widget', function( $scope ) {
	if ( $scope.data( 'shake' ) ){
		$scope.shake();
	}
} );
...
embroideryFrontend.hooks.addAction( 'frontend/element_ready/image.default', function( $scope ) {
	if ( $scope.find( 'a' ) ){
...
embroideryFrontend.hooks.addAction( 'frontend/element_ready/google-maps.satellite', function( $scope ) {
...
#### Example
 ...
 ```javascript
 embroidery.hooks.addAction( 'panel/open_editor/widget', function( panel, model, view ) {
 	if ( 'section' !== model.elType && 'column' !== model.elType ) {
 		return;
 	}
	var $element = view.$el.find( '.embroidery-selector' );
...
#### Example
 ...
 ```javascript
embroidery.hooks.addAction( 'panel/open_editor/widget/slider', function( panel, model, view ) {
	var $element = view.$el.find( '.embroidery-selector' );
