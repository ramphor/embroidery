...
class Embroidery_Test_Qunit extends WP_UnitTestCase {
	...
	public function setUp() {
		...
		add_post_meta( $GLOBALS['post']->ID, '_embroidery_edit_mode', 'builder' );

		$_REQUEST['post'] = $GLOBALS['post']->ID;
		$_REQUEST['action'] = 'embroidery';
		...
		\Embroidery\Plugin::$instance->editor->init( false );

		$html = ob_get_clean();

		$html = fix_qunit_html_urls( $html );

		$html = str_replace( wp_json_encode( add_query_arg( 'embroidery-preview', '', get_permalink( $_REQUEST['post'] ) ) ), '"./preview.html?"', $html );
