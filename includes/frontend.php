...
/**
 * Embroidery frontend class.
 *
 * Embroidery frontend handler class is responsible for initializing Embroidery in
 * the frontend.
 *
 * @since 1.0.0
 */
class Frontend {
	...
	/**
	 * Whether the page is using Embroidery.
	 *
	 * Used to determine whether the current page is using Embroidery.
	 *
	 * @since 1.0.0
	 * @access private
	 *
	 * @var bool Whether Embroidery is being used. Default is false.
	 */
	private $_has_embroidery_in_page = false;
	 ...
	 * conflicted with Embroidery while Embroidery process the content.
	 *
	 * @since 1.0.0
	 * @access private
	 *
	 ...
	 * Init.
	 *
	 * Initialize Embroidery front end. Hooks the needed actions to run Embroidery
	...
	public function init() {
		...
		if ( is_singular() && Plugin::$instance->db->is_built_with_embroidery( $this->post_id ) ) {
		...
		// Add Edit with the Embroidery in Admin Bar.
	 ...
	 * Body tag classes.
	 *
	 * Add new embroidery classes to the body tag.
	 *
	...
	public function body_class( $classes = [] ) {
		$classes[] = 'embroidery-default';

		$id = get_the_ID();

		if ( is_singular() && Plugin::$instance->db->is_built_with_embroidery( $id ) ) {
			$classes[] = 'embroidery-page embroidery-page-' . $id;
		}
	 ...
	 * Add content filter.
	 *
	 * Remove plain content and render the content generated by Embroidery.
	 *
	 * @since 1.8.0
	 * @access public
	 */
	 ...
	 *
	 * When the Embroidery generated content rendered, we remove the filter to prevent multiple
	 * accuracies. This way we make sure Embroidery renders the content only once.
	 *
	 * @since 1.8.0
	 * @access public
	 */
	...
	public function register_scripts() {
		 ...
		 *
		 * Fires before Embroidery frontend scripts are registered.
		 *
		 * @since 1.2.1
		 */
		do_action( 'embroidery/frontend/before_register_scripts' );

		$suffix = Utils::is_script_debug() ? '' : '.min';

		wp_register_script(
			'embroidery-waypoints',
			EMBROIDERY_ASSETS_URL . 'lib/waypoints/waypoints' . $suffix . '.js',
			[
				'jquery',
			],
			'4.0.2',
		...
		wp_register_script(
			'imagesloaded',
			EMBROIDERY_ASSETS_URL . 'lib/imagesloaded/imagesloaded' . $suffix . '.js',
			[
				'jquery',
			],
			'4.1.0',
		...
		wp_register_script(
			'jquery-numerator',
			EMBROIDERY_ASSETS_URL . 'lib/jquery-numerator/jquery-numerator' . $suffix . '.js',
			[
				'jquery',
			],
			'0.2.1',
		...
		wp_register_script(
			'jquery-swiper',
			EMBROIDERY_ASSETS_URL . 'lib/swiper/swiper.jquery' . $suffix . '.js',
			[
				'jquery',
			],
			'3.4.2',
		...
		wp_register_script(
			'jquery-slick',
			EMBROIDERY_ASSETS_URL . 'lib/slick/slick' . $suffix . '.js',
			[
				'jquery',
			],
			'1.6.0',
		...
		wp_register_script(
			'embroidery-frontend',
			EMBROIDERY_ASSETS_URL . 'js/frontend' . $suffix . '.js',
			[
				'embroidery-dialog',
				'embroidery-waypoints',
				'jquery-swiper',
		 ...
		 *
		 * Fires after Embroidery frontend scripts are registered.
		 *
		 * @since 1.2.1
		 */
		do_action( 'embroidery/frontend/after_register_scripts' );
	}
	...
	public function register_styles() {
		 ...
		 *
		 * Fires before Embroidery frontend styles are registered.
		 *
		 * @since 1.2.0
		 */
		do_action( 'embroidery/frontend/before_register_styles' );
		...
		wp_register_style(
			'embroidery-animations',
			EMBROIDERY_ASSETS_URL . 'css/animations.min.css',
			[],
			EMBROIDERY_VERSION
		);
		...
		wp_register_style(
			'embroidery-frontend',
			EMBROIDERY_ASSETS_URL . 'css/frontend' . $direction_suffix . $suffix . '.css',
			[],
			EMBROIDERY_VERSION
		);
		 ...
		 *
		 * Fires after Embroidery frontend styles are registered.
		 *
		 * @since 1.2.0
		 */
		do_action( 'embroidery/frontend/after_register_styles' );
	}
	...
	public function enqueue_scripts() {
		 ...
		 *
		 * Fires before Embroidery frontend scripts are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'embroidery/frontend/before_enqueue_scripts' );

		wp_enqueue_script( 'embroidery-frontend' );

		$embroidery_frontend_config = [
			'isEditMode' => Plugin::$instance->preview->is_preview_mode(),
			'settings' => SettingsManager::get_settings_frontend_config(),
			'is_rtl' => is_rtl(),
			'urls' => [
				'assets' => EMBROIDERY_ASSETS_URL,
			],
		];

		if ( is_singular() ) {
			$post = get_post();
			$embroidery_frontend_config['post'] = [
				'id' => $post->ID,
		...
		if ( Plugin::$instance->preview->is_preview_mode() ) {
			...
			$embroidery_frontend_config['elements'] = [
				'data' => (object) [],
				'editSettings' => (object) [],
		...
		wp_localize_script( 'embroidery-frontend', 'embroideryFrontendConfig', $embroidery_frontend_config );

		/**
		 * After frontend enqueue scripts.
		 *
		 * Fires after Embroidery frontend scripts are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'embroidery/frontend/after_enqueue_scripts' );
	}
	...
	public function enqueue_styles() {
		 ...
		 *
		 * Fires before Embroidery frontend styles are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'embroidery/frontend/before_enqueue_styles' );

		wp_enqueue_style( 'embroidery-icons' );
		wp_enqueue_style( 'font-awesome' );
		wp_enqueue_style( 'embroidery-animations' );
		wp_enqueue_style( 'embroidery-frontend' );
		 ...
		 *
		 * Fires after Embroidery frontend styles are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'embroidery/frontend/after_enqueue_styles' );
	}

	/**
	 * Embroidery footer scripts and styles.
	 *
	...
	public function wp_footer() {
		if ( ! $this->_has_embroidery_in_page ) {
			return;
		}
	...
	public function print_google_fonts() {
		 ...
		 */
		$print_google_fonts = apply_filters( 'embroidery/frontend/print_google_fonts', $print_google_fonts );
	 ...
	 *
	 * Used to apply the Embroidery page editor on the post content.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 ...
	 *
	 * Used to render and return the post content with all the Embroidery elements.
	 *
	...
	public function get_builder_content( $post_id, $with_css = false ) {
		...
		if ( ! Plugin::$instance->db->is_built_with_embroidery( $post_id ) ) {
			return '';
		}
		 ...
		 * @param int   $post_id The post ID.
		 */
		$data = apply_filters( 'embroidery/frontend/builder_content_data', $data, $post_id );
		...
		?>
		<div class="embroidery embroidery-<?php echo $post_id; ?>">
			<div class="embroidery-inner">
				<div class="embroidery-section-wrap">
		 ...
		 */
		$content = apply_filters( 'embroidery/frontend/the_content', $content );

		if ( ! empty( $content ) ) {
			$this->_has_embroidery_in_page = true;
		}
	...
	/**
	 * Add Embroidery menu to admin bar.
	 *
	 * Add new admin bar item only on singular pages, to display a link that
	 * allows the user to edit with Embroidery.
	 *
	...
	public function add_menu_in_admin_bar( \WP_Admin_Bar $wp_admin_bar ) {
		...
		$is_builder_mode = is_singular() && User::is_current_user_can_edit( $post_id ) && Plugin::$instance->db->is_built_with_embroidery( $post_id );
		...
		$wp_admin_bar->add_node( [
			'id' => 'embroidery_edit_page',
			'title' => __( 'Edit with Embroidery', 'embroidery' ),
	...
	public function get_builder_content_for_display( $post_id ) {
		...
		if ( get_the_ID() === (int) $post_id ) {
			...
			if ( $editor->is_edit_mode() ) {
				$content = '<div class="embroidery-alert embroidery-alert-danger">' . __( 'Invalid Data: The Template ID cannot be the same as the currently edited template. Please choose a different one.', 'embroidery' ) . '</div>';
			}
	 ...
	 *
	 * Remove WordPress default filters that conflicted with Embroidery.
	 *
	 * @since 1.5.0
	 * @access public
	 */
	 ...
	 *
	 * Restore removed WordPress filters that conflicted with Embroidery.
	 *
	 * @since 1.5.0
	 * @access private
	 */
	 ...
	 *
	 * Initializing Embroidery front end. Make sure we are not in admin, not and
	 * redirect from old URL structure of Embroidery editor.
	 *
	 * @since 1.0.0
	 * @access public
	 */
