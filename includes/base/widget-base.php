 ...
 * Widget Base.
 *
 * An abstract class to register new Embroidery widgets. It extended the
...
abstract class Widget_Base extends Element_Base {
	...
	protected static function get_default_edit_tools() {
		$widget_label = __( 'Widget', 'embroidery' );

		return [
			'duplicate' => [
				/* translators: %s: Widget Label */
				'title' => sprintf( __( 'Duplicate %s', 'embroidery' ), $widget_label ),
				'icon' => 'clone',
			],
			'remove' => [
				/* translators: %s: Widget Label */
				'title' => sprintf( __( 'Remove %s', 'embroidery' ), $widget_label ),
				'icon' => 'close',
			],
		];
	}
	...
	public function __construct( $data = [], $args = null ) {
		...
		if ( $is_type_instance ) {
			 ...
			 * Widget skin init.
			 *
			 * Fires when Embroidery widget is being initialized.
			 *
			 ...
			 */
			do_action( "embroidery/widget/{$widget_name}/skins_init", $this );
		}
	}
	...
	private function _register_skin_control() {
		...
		if ( ! empty( $skins ) ) {
			...
			if ( $this->_has_template_content ) {
				$skin_options[''] = __( 'Default', 'embroidery' );
			}
	...
	final public function print_template() {
		 ...
		 */
		$content_template = apply_filters( 'embroidery/widget/print_template', $content_template,  $this );
		...
		?>
		<script type="text/html" id="tmpl-embroidery-<?php echo static::get_type(); ?>-<?php echo esc_attr( $this->get_name() ); ?>-content">
			<?php $this->render_edit_tools(); ?>
			<div class="embroidery-widget-container">
	...
	protected function render_edit_tools() {
		...
		<div class="embroidery-element-overlay">
			<ul class="embroidery-editor-element-settings embroidery-editor-widget-settings">
				<li class="embroidery-editor-element-setting embroidery-editor-element-trigger" title="<?php printf( __( 'Edit %s', 'embroidery' ), __( 'Widget', 'embroidery' ) ); ?>">
					<i class="eicon-edit" aria-hidden="true"></i>
					<span class="embroidery-screen-only"><?php printf( __( 'Edit %s', 'embroidery' ), __( 'Widget', 'embroidery' ) ); ?></span>
				</li>
	...
	public function render_content() {
		 ...
		 *
		 * Fires before Embroidery widget is being rendered.
		 *
		 * @since 1.0.0
		 *
		 * @param Widget_Base $this The current widget.
		 */
		do_action( 'embroidery/widget/before_render_content', $this );
		...
		<div class="embroidery-widget-container">
			 ...
			 * @param Widget_Base $this           The widget.
			 */
			$widget_content = apply_filters( 'embroidery/widget/render_content', $widget_content, $this );
	 ...
	 *
	 * Embroidery saves the page content in a unique way, but it's not the way
	 ...
	 * also allows the site to keep working even if Embroidery is deactivated.
	 *
	 ...
	 * an empty string. For example Embroidery Pro Form Widget uses this method
	 * to return an empty string because there is no content to return. This way
	 * if Embroidery Pro will be deactivated there won't be any form to display.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	...
	protected function _add_render_attributes() {
		...
		$this->add_render_attribute(
			'_wrapper', 'class', [
				'embroidery-widget',
				'embroidery-widget-' . $this->get_name(),
			]
		);
	 ...
	 *
	 * The data with the HTML used always to display the data, but the Embroidery
