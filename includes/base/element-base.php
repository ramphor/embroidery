 ...
 * Element Base.
 *
 * An abstract class to register new Embroidery elements. It extended the
...
abstract class Element_Base extends Controls_Stack {
	...
	public function print_template() {
		 ...
		 */
		$content_template = apply_filters( 'embroidery/element/print_template', $content_template, $this );

		if ( empty( $content_template ) ) {
			return;
		}
		?>
		<script type="text/html" id="tmpl-embroidery-<?php echo $this->get_type(); ?>-<?php echo esc_attr( $this->get_name() ); ?>-content">
	...
	public function print_element() {
		 ...
		 *
		 * Fires before Embroidery element is rendered in the frontend.
		 *
		 ...
		 * @param Element_Base $this The element.
		 */
		do_action( "embroidery/frontend/{$element_type}/before_render", $this );
		 ...
		 *
		 * Fires after Embroidery element was rendered in the frontend.
		 *
		 * The dynamic portion of the hook name, `$element_type`, refers to the element type.
		 *
		 * @since 1.0.0
		 ...
		 * @param Element_Base $this The element.
		 */
		do_action( "embroidery/frontend/{$element_type}/after_render", $this );
	}
	 ...
	 * class for each HTML element. This way Embroidery can set custom styles for
	 * each element.
	 *
	 * @since 1.0.0
	 * @access public
	...
	public function get_unique_selector() {
		return '.embroidery-element-' . $this->get_id();
	}
	...
	protected function _add_render_attributes() {
		...
		if ( ! empty( $settings['animation'] ) || ! empty( $settings['_animation'] ) ) {
			...
			$this->add_render_attribute( '_wrapper', 'class', 'embroidery-invisible' );
		}
	...
	private function _get_child_type( $element_data ) {
		 ...
		 */
		$child_type = apply_filters( 'embroidery/element/get_child_type', $child_type, $element_data, $this );
	 ...
	 * way Embroidery renders data when initializing elements.
	 *
	 * @since 1.0.0
	 * @access public
	 *
