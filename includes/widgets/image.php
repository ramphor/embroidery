...
/**
 * Embroidery image widget.
 *
 * Embroidery widget that displays an image into the page.
 *
 * @since 1.0.0
 */
class Widget_Image extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'default' => '',
				'placeholder' => __( 'Enter your image caption', 'embroidery' ),
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .embroidery-image img' => 'max-width: {{SIZE}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .embroidery-image img' => 'opacity: {{SIZE}};',
				],
			]
		);
		...
		$this->add_group_control(
			...
			[
				'name' => 'image_border',
				'selector' => '{{WRAPPER}} .embroidery-image img',
				'separator' => 'before',
			]
		);
		...
		$this->add_responsive_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}} .embroidery-image img' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
	...
	protected function render() {
		...
		$this->add_render_attribute( 'wrapper', 'class', 'embroidery-image' );

		if ( ! empty( $settings['shape'] ) ) {
			$this->add_render_attribute( 'wrapper', 'class', 'embroidery-image-shape-' . $settings['shape'] );
		}
	...
	protected function _content_template() {
		...
		<# if ( '' !== settings.image.url ) {
			...
			var image_url = embroidery.imagesManager.getImageUrl( image );
			...
			#><div class="embroidery-image{{ settings.shape ? ' embroidery-image-shape-' + settings.shape : '' }}"><#
			var imgClass = '',
				hasCaption = '' !== settings.caption;

			if ( '' !== settings.hover_animation ) {
				imgClass = 'embroidery-animation-' + settings.hover_animation;
			}
			...
			if ( link_url ) {
					#><a class="embroidery-clickable" data-embroidery-open-lightbox="{{ settings.open_lightbox }}" href="{{ link_url }}"><#
			}
