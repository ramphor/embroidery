...
/**
 * Embroidery sidebar widget.
 *
 * Embroidery widget that insert any sidebar into the page.
 *
 * @since 1.0.0
 */
class Widget_Sidebar extends Widget_Base {
	...
	protected function _register_controls() {
		...
		if ( ! $wp_registered_sidebars ) {
			$options[''] = __( 'No sidebars were found', 'embroidery' );
		} else {
			$options[''] = __( 'Choose Sidebar', 'embroidery' );
		...
		$this->start_controls_section(
			'section_sidebar',
			[
				'label' => __( 'Sidebar', 'embroidery' ),
			]
		);

		$this->add_control( 'sidebar', [
			'label' => __( 'Choose Sidebar', 'embroidery' ),
