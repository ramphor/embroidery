...
abstract class CSS_File {
	...
	const FILE_BASE_DIR = '/embroidery/css';
	...
	*/
	...
	protected function use_external_file() {
		return 'internal' !== get_option( 'embroidery_css_print_method' );
	}
	...
	*/
	public function enqueue() {
		 ...
		 * CSS file enqueue.
		 *
		 * Fires when CSS file is enqueued on Embroidery.
		 *
		 ...
		 */
		do_action( "embroidery/{$name}-css-file/enqueue", $this );
	}
			...
			if ( ! empty( $control['style_fields'] ) ) {
				foreach ( $values[ $control['name'] ] as $field_value ) {
					$this->add_controls_stack_style_rules(
						...
						array_merge( $replacements, [ '.embroidery-repeater-item-' . $field_value['_id'] ] )
					);
				}
			}
	...
	*/
	protected function parse_css() {
		 ...
		 * CSS file parse.
		 *
		 * Fires when CSS file is parsed on Embroidery.
		 *
		 * The dynamic portion of the hook name, `$name`, refers to the CSS file name.
		 *
		 * @since 1.2.0
		 ...
		 * @param CSS_File $this The current CSS file.
		 */
		do_action( "embroidery/{$name}-css-file/parse", $this );
