...
/**
 * Embroidery API class.
 *
 * Embroidery API handler class is responsible for communicating with Embroidery
...
class Api {
	 ...
	 */
	public static $api_info_url = 'http://my.embroidery.com/api/v1/info/';
	 ...
	 */
	private static $api_feedback_url = 'http://my.embroidery.com/api/v1/feedback/';
	 ...
	 */
	private static $api_get_template_content_url = 'http://my.embroidery.com/api/v1/templates/%d';
	...
	private static function _get_info_data( $force_update = false ) {
		$cache_key = 'embroidery_remote_info_api_data_' . EMBROIDERY_VERSION;
		...
		if ( $force_update || false === $info_data ) {
			...
			$response = wp_remote_post( self::$api_info_url, [
				...
				'body' => [
					...
					'api_version' => EMBROIDERY_VERSION,
			...
			if ( isset( $info_data['templates'] ) ) {
				update_option( 'embroidery_remote_info_templates_data', $info_data['templates'], 'no' );

				unset( $info_data['templates'] );
			}

			if ( isset( $info_data['feed'] ) ) {
				update_option( 'embroidery_remote_info_feed_data', $info_data['feed'], 'no' );
	...
	public static function get_templates_data( $force_update = false ) {
		...
		$templates = get_option( 'embroidery_remote_info_templates_data' );
	...
	public static function get_feed_data( $force_update = false ) {
		...
		$feed = get_option( 'embroidery_remote_info_feed_data' );
	...
	public static function get_template_content( $template_id ) {
		 ...
		 */
		$body_args = apply_filters( 'embroidery/api/get_templates/body_args', $body_args );
	 ...
	 * Send Feedback.
	 *
	 * Fires a request to Embroidery server with the feedback data.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	 ...
	 * Ajax reset API data.
	 *
	 * Reset Embroidery library API data using an ajax call.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	...
	public static function ajax_reset_api_data() {
		check_ajax_referer( 'embroidery_reset_library', '_nonce' );
	...
	public static function init() {
		add_action( 'wp_ajax_embroidery_reset_library', [ __CLASS__, 'ajax_reset_api_data' ] );
	}
}
