...
/**
 * Embroidery autoloader class.
 *
 * Embroidery autoloader handler class is responsible for loading the different
...
class Autoloader {
	 ...
	 * Classes map.
	 *
	 * Maps Embroidery classes to file names.
	 *
	 * @since 1.6.0
	 * @access private
	 * @static
	 *
	 * @var array Classes used by embroidery.
	 */
	 ...
	 * Classes aliases.
	 *
	 * Maps Embroidery classes to aliases.
	 *
	 * @since 1.6.0
	 * @access private
	 * @static
	...
	private static function load_class( $relative_class_name ) {
		if ( isset( self::$classes_map[ $relative_class_name ] ) ) {
			$filename = EMBROIDERY_PATH . '/' . self::$classes_map[ $relative_class_name ];
		} else {
			$filename = strtolower(
				preg_replace(
					[ '/([a-z])([A-Z])/', '/_/', '/\\\/' ],
			...
			$filename = EMBROIDERY_PATH . $filename . '.php';
		}
