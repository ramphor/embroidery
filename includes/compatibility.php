...
/**
 * Embroidery compatibility class.
 *
 * Embroidery compatibility handler class is responsible for compatibility with
...
class Compatibility {
	 ...
	 * Register actions.
	 *
	 * Run Embroidery compatibility with external plugins using custom filters and
	 * actions.
	 *
	 * @since 1.0.0
	 * @access public
	...
	*/
	public static function register_actions() {
		...
		if ( is_admin() ) {
			...
			if ( function_exists( 'gutenberg_init' ) ) {
				...
				add_filter( 'embroidery/utils/exit_to_dashboard_url', [ __CLASS__, 'exit_to_classic_editor' ] );
			}
		}
	}
	 ...
	 * Init.
	 *
	 * Initialize Embroidery compatibility with external plugins.
	 *
	...
	public static function add_new_button_to_gutenberg() {
		...
		<script type="text/javascript">
			document.addEventListener( 'DOMContentLoaded', function() {
				...
				dropdown.insertAdjacentHTML( 'afterbegin', '<a href="' + url + '">Embroidery</a>' );
			} );
		</script>
		<?php
	}
	...
	*/
	public static function init() {
		...
		if ( defined( 'NGG_PLUGIN_VERSION' ) ) {
			add_filter( 'embroidery/utils/get_edit_link', function( $edit_link ) {
		...
		// Add the description (content) tab for a new product, so it can be edited with Embroidery.
		add_filter( 'woocommerce_product_tabs', function( $tabs ) {
			if ( ! isset( $tabs['description'] ) && Plugin::$instance->preview->is_preview_mode() ) {
				$post = get_post();
				if ( empty( $post->post_content ) ) {
					$tabs['description'] = [
						'title' => __( 'Description', 'embroidery' ),
						'priority' => 10,
		...
		if ( class_exists( 'domain_map' ) ) {
			add_filter( 'embroidery/utils/preview_url', function( $preview_url ) {
		...
		// Copy embroidery data while polylang creates a translation copy
	 ...
	 * Save polylang meta.
	 *
	 * Copy embroidery data while polylang creates a translation copy. Fired by
	...
	*/
	public static function save_polylang_meta( $keys, $sync, $from, $to ) {
		...
		if ( ! $sync ) {
			Plugin::$instance->db->copy_embroidery_meta( $from, $to );
		}
	 ...
	 *
	 * Normalize Embroidery post meta on import, We need the `wp_slash` in order
	 ...
	 *
	 * Normalize Embroidery post meta on import with the new WP_importer, We need
	...
	public static function on_wxr_importer_pre_process_post_meta( $post_meta ) {
		if ( '_embroidery_data' === $post_meta['key'] ) {
