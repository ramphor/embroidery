...
/**
 * Embroidery section element class.
 *
 * Embroidery repeater handler class is responsible for initializing the section
 * element.
 *
 * @since 1.0.0
 */
class Element_Section extends Element_Base {
	 ...
	 * default by Embroidery. Default is an empty array.
	 *
	 * Note that when the user creates a section he can define custom sizes for
	 * the columns. But Embroidery sets default values for predefined columns.
	 *
	...
	protected static function get_default_edit_tools() {
		$section_label = __( 'Section', 'embroidery' );

		return [
			'duplicate' => [
				'title' => sprintf( __( 'Duplicate %s', 'embroidery' ), $section_label ),
				'icon' => 'clone',
			],
			'add' => [
				'title' => sprintf( __( 'Add %s', 'embroidery' ), $section_label ),
				'icon' => 'plus',
			],
			'save' => [
				'title' => sprintf( __( 'Save %s', 'embroidery' ), $section_label ),
				'icon' => 'save',
			],
			'remove' => [
				'title' => sprintf( __( 'Remove %s', 'embroidery' ), $section_label ),
				'icon' => 'close',
			],
		];
	}
	 ...
	 *
	 * Note that Embroidery sections have default section presets but the user
	...
	protected function _register_controls() {
		...
		$this->add_control(
			'stretch_section',
			[
				'label' => __( 'Stretch Section', 'embroidery' ),
				'type' => Controls_Manager::SWITCHER,
				'default' => '',
				'return_value' => 'section-stretched',
				'prefix_class' => 'embroidery-',
				'render_type' => 'template',
				'hide_in_inner' => true,
				'description' => __( 'Stretch the section to the full width of the page using JS.', 'embroidery' ) . sprintf( ' <a href="%s" target="_blank">%s</a>', 'https://go.embroidery.com/stretch-section/', __( 'Learn more.', 'embroidery' ) ),
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'default' => 'boxed',
				'options' => [
					'boxed' => __( 'Boxed', 'embroidery' ),
					'full_width' => __( 'Full Width', 'embroidery' ),
				],
				'prefix_class' => 'embroidery-section-',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} > .embroidery-container' => 'max-width: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'layout' => [ 'boxed' ],
				],
		...
		$this->add_control(
			'gap',
			[
				'label' => __( 'Columns Gap', 'embroidery' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'default',
				'options' => [
					'default' => __( 'Default', 'embroidery' ),
					'no' => __( 'No Gap', 'embroidery' ),
					'narrow' => __( 'Narrow', 'embroidery' ),
					'extended' => __( 'Extended', 'embroidery' ),
					'wide' => __( 'Wide', 'embroidery' ),
					'wider' => __( 'Wider', 'embroidery' ),
				],
			]
		);
		...
		$this->add_control(
			'height',
			[
				'label' => __( 'Height', 'embroidery' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'default',
				'options' => [
					'default' => __( 'Default', 'embroidery' ),
					'full' => __( 'Fit To Screen', 'embroidery' ),
					'min-height' => __( 'Min Height', 'embroidery' ),
				],
				'prefix_class' => 'embroidery-section-height-',
				'hide_in_inner' => true,
			]
		);
		...
		$this->add_responsive_control(
			'custom_height',
			[
				'label' => __( 'Minimum Height', 'embroidery' ),
				'type' => Controls_Manager::SLIDER,
				'default' => [
					'size' => 400,
				],
				...
				'size_units' => [ 'px', 'vh' ],
				'selectors' => [
					'{{WRAPPER}} > .embroidery-container' => 'min-height: {{SIZE}}{{UNIT}};',
				],
				'condition' => [
					'height' => [ 'min-height' ],
				],
		...
		$this->add_control(
			'column_position',
			[
				'label' => __( 'Column Position', 'embroidery' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'middle',
				'options' => [
					'stretch' => __( 'Stretch', 'embroidery' ),
					'top' => __( 'Top', 'embroidery' ),
					'middle' => __( 'Middle', 'embroidery' ),
					'bottom' => __( 'Bottom', 'embroidery' ),
				],
				'prefix_class' => 'embroidery-section-items-',
				'condition' => [
		...
		$this->add_control(
			...
			[
				...
				'prefix_class' => 'embroidery-section-content-',
			]
		);
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}} > .embroidery-background-overlay',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} > .embroidery-background-overlay' => 'opacity: {{SIZE}};',
				],
				'condition' => [
		...
		$this->add_group_control(
			...
			[
				...
				'selector' => '{{WRAPPER}}:hover > .embroidery-background-overlay',
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}}:hover > .embroidery-background-overlay' => 'opacity: {{SIZE}};',
				],
				'condition' => [
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}}, {{WRAPPER}} > .embroidery-background-overlay' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'size_units' => [ 'px', '%' ],
				'selectors' => [
					'{{WRAPPER}}:hover, {{WRAPPER}}:hover > .embroidery-background-overlay' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					...
					'{{WRAPPER}} > .embroidery-background-overlay' => 'transition: background {{background_overlay_hover_transition.SIZE}}s, border-radius {{SIZE}}s, opacity {{background_overlay_hover_transition.SIZE}}s',
				],
			]
		);
		...
		$this->start_controls_section(
			...
			[
				'label' => __( 'Shape Divider', 'embroidery' ),
				'tab' => Controls_Manager::TAB_STYLE,
			]
		);
		...
		foreach ( [
			...
			$this->add_control(
				...
				[
					...
					'selectors' => [
						"{{WRAPPER}} > .embroidery-shape-$side .embroidery-shape-fill" => 'fill: {{UNIT}};',
					],
				]
			);
			...
			$this->add_responsive_control(
				...
				[
					...
					'selectors' => [
						"{{WRAPPER}} > .embroidery-shape-$side svg" => 'width: calc({{SIZE}}{{UNIT}} + 1.3px)',
					],
				]
			);
			...
			$this->add_responsive_control(
				...
				[
					...
					'selectors' => [
						"{{WRAPPER}} > .embroidery-shape-$side svg" => 'height: {{SIZE}}{{UNIT}};',
					],
				]
			);
			...
			$this->add_control(
				...
				[
					'label' => __( 'Flip', 'embroidery' ),
					'type' => Controls_Manager::SWITCHER,
					'condition' => [
						"shape_divider_$side" => array_keys( Shapes::filter_shapes( 'has_flip' ) ),
					],
					'selectors' => [
						"{{WRAPPER}} > .embroidery-shape-$side .embroidery-shape-fill" => 'transform: rotateY(180deg)',
					],
				]
			);
			...
			$this->add_control(
				...
				[
					'label' => __( 'Bring to Front', 'embroidery' ),
					'type' => Controls_Manager::SWITCHER,
					'selectors' => [
						"{{WRAPPER}} > .embroidery-shape-$side" => 'z-index: 2; pointer-events: none',
					],
					'condition' => [
						"shape_divider_$side!" => '',
					],
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} > .embroidery-container' => 'text-align: {{VALUE}};',
				],
			]
		);
		...
		$this->add_control(
			...
			[
				'label' => __( 'Reverse Columns', 'embroidery' ),
				'type' => Controls_Manager::SWITCHER,
				'default' => '',
				'prefix_class' => 'embroidery-',
				'return_value' => 'reverse-mobile',
				'description' => __( 'Reverse column order - When on mobile, the column order is reversed, so the last column appears on top and vice versa.', 'embroidery' ),
			]
		);
		...
		$this->add_control(
			'heading_visibility',
			[
				'label' => __( 'Visibility', 'embroidery' ),
		...
		$this->add_control(
			...
			[
				'raw' => __( 'Attention: The display settings (show/hide for mobile, tablet or desktop) will only take effect once you are on the preview or live page, and not while you\'re in editing mode in Embroidery.', 'embroidery' ),
				'type' => Controls_Manager::RAW_HTML,
				'content_classes' => 'embroidery-descriptor',
			]
		);
		...
		$this->add_control(
			'hide_desktop',
			[
				'label' => __( 'Hide On Desktop', 'embroidery' ),
				'type' => Controls_Manager::SWITCHER,
				'default' => '',
				'prefix_class' => 'embroidery-',
				'label_on' => __( 'Hide', 'embroidery' ),
				'label_off' => __( 'Show', 'embroidery' ),
		...
		$this->add_control(
			'hide_tablet',
			[
				'label' => __( 'Hide On Tablet', 'embroidery' ),
				'type' => Controls_Manager::SWITCHER,
				'default' => '',
				'prefix_class' => 'embroidery-',
				'label_on' => __( 'Hide', 'embroidery' ),
		...
		$this->add_control(
			'hide_mobile',
			[
				'label' => __( 'Hide On Mobile', 'embroidery' ),
				'type' => Controls_Manager::SWITCHER,
				'default' => '',
				'prefix_class' => 'embroidery-',
				'label_on' => __( 'Hide', 'embroidery' ),
	...
	protected function render_edit_tools() {
		...
		<div class="embroidery-element-overlay">
			<ul class="embroidery-editor-element-settings embroidery-editor-section-settings">
				<li class="embroidery-editor-element-setting embroidery-editor-element-trigger embroidery-active" title="<?php printf( __( 'Edit %s', 'embroidery' ),  __( 'Section', 'embroidery' ) ); ?>">
					<i class="eicon-section" aria-hidden="true"></i>
					<span class="embroidery-screen-only"><?php printf( __( 'Edit %s', 'embroidery' ), __( 'Section', 'embroidery' ) ); ?></span>
				</li>
	...
	protected function _content_template() {
		?>
		<div class="embroidery-background-video-container embroidery-hidden-phone">
			<div class="embroidery-background-video-embed"></div>
			<video class="embroidery-background-video-hosted" autoplay loop muted></video>
		</div>
		<div class="embroidery-background-video-fallback"></div>
		<div class="embroidery-background-overlay"></div>
		<div class="embroidery-shape embroidery-shape-top"></div>
		<div class="embroidery-shape embroidery-shape-bottom"></div>
		<div class="embroidery-container embroidery-column-gap-{{ settings.gap }}">
			<div class="embroidery-row"></div>
		</div>
		<?php
	}
	...
	public function before_render() {
		...
		<<?php echo $this->get_html_tag() . ' ' . $this->get_render_attribute_string( '_wrapper' ); ?>>
				...
				if ( $settings['background_video_link'] ) :
					...
					<div class="embroidery-background-video-container embroidery-hidden-phone">
						<?php if ( $video_properties ) : ?>
							<div class="embroidery-background-video-embed"></div>
						<?php else : ?>
							<video class="embroidery-background-video-hosted embroidery-html5-video" autoplay loop muted></video>
						<?php endif; ?>
					</div>
				<?php
				endif;
			...
			?>
			<div class="embroidery-container embroidery-column-gap-<?php echo esc_attr( $settings['gap'] ); ?>">
				<div class="embroidery-row">
		<?php
	}
	...
	protected function _add_render_attributes() {
		...
		$this->add_render_attribute(
			'_wrapper', 'class', [
				'embroidery-section',
				'embroidery-' . $section_type . '-section',
			]
		);
	...
	private function print_shape_divider( $side ) {
		...
		?>
		<div class="embroidery-shape embroidery-shape-<?php echo $side; ?>" data-negative="<?php echo var_export( $negative ); ?>">
