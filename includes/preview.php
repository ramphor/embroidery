...
/**
 * Embroidery preview class.
 *
 * Embroidery preview handler class is responsible for initializing Embroidery in
 * preview mode.
 *
 * @since 1.0.0
 */
class Preview {
	 ...
	 * Init.
	 *
	 * Initialize Embroidery preview mode.
	 *
	...
	public function init() {
		 ...
		 * Preview init.
		 *
		 * Fires on Embroidery preview init, after Embroidery preview has finished
		 ...
		 */
		do_action( 'embroidery/preview/init', $this );
	}
	...
	public function is_preview_mode() {
		...
		if ( ! isset( $_GET['embroidery-preview'] ) ) {
			return false;
		}
	...
	public function builder_wrapper() {
		return '<div id="embroidery" class="embroidery embroidery-edit-mode"></div>';
	}
	...
	private function enqueue_styles() {
		...
		wp_register_style(
			'editor-preview',
			EMBROIDERY_ASSETS_URL . 'css/editor-preview' . $direction_suffix . $suffix . '.css',
			[],
			EMBROIDERY_VERSION
		);
		 ...
		 *
		 * Fires after Embroidery preview styles are enqueued.
		 *
		 * @since 1.0.0
		 */
		do_action( 'embroidery/preview/enqueue_styles' );
	}
	...
	private function enqueue_scripts() {
		...
		wp_enqueue_script(
			'embroidery-inline-editor',
			EMBROIDERY_ASSETS_URL . 'lib/inline-editor/js/inline-editor' . $suffix . '.js',
			[],
			'',
			true
		);
		 ...
		 *
		 * Fires after Embroidery preview scripts are enqueued.
		 *
		 * @since 1.5.4
		 */
		do_action( 'embroidery/preview/enqueue_scripts' );
	}

	/**
	 * Preview constructor.
	 *
	 * Initializing Embroidery preview.
	 *
	 * @since 1.0.0
	 * @access public
	 */
