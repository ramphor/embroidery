...
class Control_Gallery extends Base_Data_Control {
	 ...
	 * Embroidery template JSON file, and replacing the old data.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	...
	public function content_template() {
		...
		<div class="embroidery-control-field">
			<div class="embroidery-control-input-wrapper">
				...
				<# } #>
				<div class="embroidery-control-media">
					<div class="embroidery-control-gallery-status">
						<span class="embroidery-control-gallery-status-title">
							<# if ( data.controlValue.length ) {
								print( embroidery.translate( 'gallery_images_selected', [ data.controlValue.length ] ) );
							} else { #>
								<?php _e( 'No Images Selected', 'embroidery' ); ?>
							<# } #>
						</span>
						<span class="embroidery-control-gallery-clear">(<?php _e( 'Clear', 'embroidery' ); ?>)</span>
					</div>
					<div class="embroidery-control-gallery-thumbnails">
						<# _.each( data.controlValue, function( image ) { #>
							<div class="embroidery-control-gallery-thumbnail" style="background-image: url({{ image.url }})"></div>
						<# } ); #>
					</div>
					<button class="embroidery-button embroidery-control-gallery-add"><?php _e( '+ Add Images', 'embroidery' ); ?></button>
				</div>
			</div>
		</div>
		<?php
