...
class Group_Control_Image_Size extends Group_Control_Base {
	...
	public static function get_attachment_image_html( $settings, $setting_key = 'image' ) {
		...
		$image_class = ! empty( $settings['hover_animation'] ) ? 'embroidery-animation-' . $settings['hover_animation'] : '';
	...
	public static function get_attachment_image_src( $attachment_id, $group_name, array $settings ) {
		...
		if ( 'custom' !== $size ) {
			...
			require_once( EMBROIDERY_PATH . 'includes/libraries/bfi-thumb/bfi-thumb.php' );
	...
	protected function init_fields() {
		...
		$fields['size'] = [
			'label' => _x( 'Image Size', 'Image Size Control', 'embroidery' ),
			'type' => Controls_Manager::SELECT,
			'label_block' => false,
		];

		$fields['custom_dimension'] = [
			'label' => _x( 'Image Dimension', 'Image Size Control', 'embroidery' ),
			'type' => Controls_Manager::IMAGE_DIMENSIONS,
			'description' => __( 'You can crop the original image size to any custom size. You can also set a single value for height or width in order to keep the original size ratio.', 'embroidery' ),
			'condition' => [
				'size' => 'custom',
			],
			'separator' => 'none',
	...
	private function _get_image_sizes() {
		...
		$image_sizes['full'] = _x( 'Full', 'Image Size Control', 'embroidery' );

		if ( ! empty( $args['include']['custom'] ) || ! in_array( 'custom', $args['exclude'] ) ) {
			$image_sizes['custom'] = _x( 'Custom', 'Image Size Control', 'embroidery' );
		}
