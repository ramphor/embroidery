...
class Control_Color extends Base_Data_Control {
	...
	public function enqueue() {
		...
		wp_register_script(
			...
			EMBROIDERY_ASSETS_URL . 'lib/wp-color-picker/wp-color-picker-alpha' . $suffix . '.js',
			[
				'wp-color-picker',
			],
			'2.0.1',
	...
	public function content_template() {
		...
		<div class="embroidery-control-field">
			<label class="embroidery-control-title">
				<# if ( data.label ) { #>
					{{{ data.label }}}
				<# } #>
				<# if ( data.description ) { #>
					<span class="embroidery-control-field-description">{{{ data.description }}}</span>
				<# } #>
			</label>
			<div class="embroidery-control-input-wrapper">
				<input data-setting="{{ name }}" type="text" placeholder="<?php echo esc_attr( 'Hex/rgba', 'embroidery' ); ?>" {{ defaultValue }}{{ dataAlpha }} />
			</div>
		</div>
		<?php
	}
