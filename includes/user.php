...
/**
 * Embroidery user class.
 *
 * Embroidery user handler class is responsible for checking if the user can edit
 * with Embroidery and displaying different admin notices.
 *
 * @since 1.0.0
 */
class User {
	 ...
	 */
	const ADMIN_NOTICES_KEY = 'embroidery_admin_notices';

	/**
	 * Init.
	 *
	 * Initialize Embroidery user.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	...
	public static function init() {
		add_action( 'wp_ajax_embroidery_set_admin_notice_viewed', [ __CLASS__, 'ajax_set_admin_notice_viewed' ] );
	}
	...
	public static function is_current_user_can_edit( $post_id = 0 ) {
		...
		$exclude_roles = get_option( 'embroidery_exclude_user_roles', [] );
	...
	public static function is_current_user_can_edit_post_type( $post_type ) {
		...
		if ( ! post_type_supports( $post_type, 'embroidery' ) ) {
			return false;
		}
	 ...
	 *
	 * Fired by `wp_ajax_embroidery_set_admin_notice_viewed` action.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
