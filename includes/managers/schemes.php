...
/**
 * Embroidery scheme manager class.
 *
 * Embroidery scheme manager handler class is responsible for registering and
...
class Schemes_Manager {
	 ...
	 * Ajax apply scheme.
	 *
	 * Ajax handler for Embroidery apply_scheme.
	 *
	 * Fired by `wp_ajax_embroidery_apply_scheme` action.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	...
	public static function get_enabled_schemes() {
		if ( null === self::$_enabled_schemes ) {
			...
			foreach ( self::$_schemes_types as $schemes_type => $scheme_class ) {
				if ( 'yes' === get_option( 'embroidery_disable_' . $schemes_type . '_schemes' ) ) {
					continue;
				}
			 ...
			 */
			self::$_enabled_schemes = apply_filters( 'embroidery/schemes/enabled_schemes', $enabled_schemes );
		}
	 ...
	 *
	 * Initializing Embroidery schemes manager and register default schemes.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function __construct() {
		...
		add_action( 'wp_ajax_embroidery_apply_scheme', [ $this, 'ajax_apply_scheme' ] );
	}
}
