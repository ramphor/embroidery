...
/**
 * Embroidery elements manager class.
 *
 * Embroidery elements manager handler class is responsible for registering and
...
class Elements_Manager {
	 ...
	 *
	 * Initializing Embroidery elements manager.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function __construct() {
		...
		add_action( 'wp_ajax_embroidery_save_builder', [ $this, 'ajax_save_builder' ] );
		add_action( 'wp_ajax_embroidery_discard_changes', [ $this, 'ajax_discard_changes' ] );
	}
	 ...
	 *
	 * Ajax handler for Embroidery discard_changes. Handles the discarded changes
	 * in the builder by deleting autosaved revisions.
	 *
	 * Fired by `wp_ajax_embroidery_discard_changes` action.
	 *
	 * @since 1.9.0
	 * @access public
	 */
	 ...
	 * Ajax save builder.
	 *
	 * Ajax handler for Embroidery save_builder. Handles the saved data returned
	 * by the builder.
	 *
	 * Fired by `wp_ajax_embroidery_save_builder` action.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function ajax_save_builder() {
		 ...
		 */
		$return_data = apply_filters( 'embroidery/ajax_save_builder/return_data', $return_data, $post_id );
	 ...
	 * Init elements.
	 *
	 * Initialize Embroidery elements by registering the supported elements.
	 * Embroidery supports by default `section` element and `column` element.
	 *
	 * @since 1.0.0
	 * @access private
	 */
	private function _init_elements() {
		 ...
		 *
		 * Fires after Embroidery elements are registered.
		 *
		 * @since 1.0.0
		 */
		do_action( 'embroidery/elements/elements_registered' );
	}
	...
	private function init_categories() {
		$this->_categories = [
			'basic' => [
				'title' => __( 'Basic', 'embroidery' ),
				'icon' => 'eicon-font',
			],
			'general-elements' => [
				'title' => __( 'General Elements', 'embroidery' ),
				'icon' => 'eicon-font',
			],
			'pojo' => [
				'title' => __( 'Pojo Themes', 'embroidery' ),
				'icon' => 'eicon-pojome',
			],
			'wordpress' => [
				'title' => __( 'WordPress', 'embroidery' ),
	 ...
	 * Require files.
	 *
	 * Require Embroidery element base class and column, section and repeater
	 * elements.
	 *
	 * @since 1.0.0
	 * @access private
	...
	private function require_files() {
		require_once EMBROIDERY_PATH . 'includes/base/element-base.php';

		require EMBROIDERY_PATH . 'includes/elements/column.php';
		require EMBROIDERY_PATH . 'includes/elements/section.php';
		require EMBROIDERY_PATH . 'includes/elements/repeater.php';
	}
}
