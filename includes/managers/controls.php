...
/**
 * Embroidery controls manager class.
 *
 * Embroidery controls manager handler class is responsible for registering and
...
class Controls_Manager {
	...
	private static function init_tabs() {
		self::$tabs = [
			self::TAB_CONTENT => __( 'Content', 'embroidery' ),
			self::TAB_STYLE => __( 'Style', 'embroidery' ),
			self::TAB_ADVANCED => __( 'Advanced', 'embroidery' ),
			self::TAB_RESPONSIVE => __( 'Responsive', 'embroidery' ),
			self::TAB_LAYOUT => __( 'Layout', 'embroidery' ),
			self::TAB_SETTINGS => __( 'Settings', 'embroidery' ),
		];

		self::$tabs = Utils::apply_filters_deprecated( 'embroidery/controls/get_available_tabs_controls', [ self::$tabs ], '1.6.0', '`' . __CLASS__ . '::add_tab( $tab_name, $tab_title )`' );
	}
	 ...
	 * `embroidery/controls/controls_registered` action.
	 *
	 * @since 1.0.0
	 * @access private
	 */
	private function register_controls() {
		...
		foreach ( $available_controls as $control_id ) {
			...
			$control_filename = EMBROIDERY_PATH . "includes/controls/{$control_filename}.php";
		 ...
		 *
		 * Fires after Embroidery controls are registered.
		 *
		 * @since 1.0.0
		 *
		 * @param Controls_Manager $this The controls manager.
		 */
		do_action( 'embroidery/controls/controls_registered', $this );
	}
	 ...
	 * version of embroidery uses this method to display an upgrade message to
	 * Embroidery Pro.
	 *
	 * @since 1.0.0
	 * @access public
	...
	public function add_custom_css_controls( $element ) {
		$element->start_controls_section(
			...
			[
				'label' => __( 'Custom CSS', 'embroidery' ),
		...
		$element->add_control(
			...
			[
				...
				'raw' => '<div class="embroidery-panel-nerd-box">
						<i class="embroidery-panel-nerd-box-icon eicon-hypster" aria-hidden="true"></i>
						<div class="embroidery-panel-nerd-box-title">' .
							__( 'Meet Our Custom CSS', 'embroidery' ) .
						'</div>
						<div class="embroidery-panel-nerd-box-message">' .
							__( 'Custom CSS lets you add CSS code to any widget, and see it render live right in the editor.', 'embroidery' ) .
						'</div>
						<div class="embroidery-panel-nerd-box-message">' .
							__( 'This feature is only available on Embroidery Pro.', 'embroidery' ) .
						'</div>
						<a class="embroidery-panel-nerd-box-link embroidery-button embroidery-button-default embroidery-go-pro" href="' . Utils::get_pro_link( 'https://embroidery.com/pro/?utm_source=panel-custom-css&utm_campaign=gopro&utm_medium=wp-dash' ) . '" target="_blank">' .
							__( 'Go Pro', 'embroidery' ) .
						'</a>
						</div>',
			]
		);
