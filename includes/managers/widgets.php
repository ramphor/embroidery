...
/**
 * Embroidery widgets manager class.
 *
 * Embroidery widgets manager handler class is responsible for registering and
 * initializing all the supported Embroidery widgets.
 *
 * @since 1.0.0
 */
class Widgets_Manager {
	 ...
	 * Init widgets.
	 *
	 * Initialize Embroidery widgets manager. Include all the the widgets files
	 * and register each Embroidery and WordPress widget.
	 *
	 * @since 1.0.0
	 * @access private
	 */
	private function _init_widgets() {
		...
		foreach ( $build_widgets_filename as $widget_filename ) {
			include( EMBROIDERY_PATH . 'includes/widgets/' . $widget_filename . '.php' );
		 ...
		 *
		 * Fires after Embroidery widgets are registered.
		 *
		 * @since 1.0.0
		 *
		 * @param Widgets_Manager $this The widgets manager.
		 */
		do_action( 'embroidery/widgets/widgets_registered', $this );
	}
	 ...
	 *
	 * Exclude the widgets that are in Embroidery widgets black list. Theme and
	...
	private function _register_wp_widgets() {
		 ...
		 */
		$black_list = apply_filters( 'embroidery/widgets/black_list', $black_list );
		...
		foreach ( $wp_widget_factory->widgets as $widget_class => $widget_obj ) {
			...
			$embroidery_widget_class = __NAMESPACE__ . '\Widget_WordPress';

			$this->register_widget_type(
				new $embroidery_widget_class( [], [
	 ...
	 * Require files.
	 *
	 * Require Embroidery widget base class.
	 *
	 * @since 1.0.0
	 * @access private
	 */
	private function _require_files() {
		require EMBROIDERY_PATH . 'includes/base/widget-base.php';
	}
	 ...
	 * @access public
	 *
	 * @param Widget_Base $widget Embroidery widget.
	 *
	 ...
	 * Ajax render widget.
	 *
	 * Ajax handler for Embroidery render_widget.
	 *
	 * Fired by `wp_ajax_embroidery_render_widget` action.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	 ...
	 *
	 * Ajax handler for Embroidery editor get_wp_widget_form.
	 *
	 * Fired by `wp_ajax_embroidery_editor_get_wp_widget_form` action.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	 ...
	 *
	 * Initializing Embroidery widgets manager.
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function __construct() {
		...
		add_action( 'wp_ajax_embroidery_render_widget', [ $this, 'ajax_render_widget' ] );
		add_action( 'wp_ajax_embroidery_editor_get_wp_widget_form', [ $this, 'ajax_get_wp_widget_form' ] );
	}
}
