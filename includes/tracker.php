...
/**
 * Embroidery tracker class.
 *
 * Embroidery tracker handler class is responsible for sending anonymous plugin
 * data to Embroidery servers for users that actively allowed data tracking.
 *
 * @since 1.0.0
 */
class Tracker {
	 ...
	 * @var string API URL.
	 */
	private static $_api_url = 'http://my.embroidery.com/api/v1/tracker/';

	/**
	 * Init.
	 *
	 * Initialize Embroidery tracker.
	 *
	 * @since 1.0.0
	 * @access public
	 * @static
	...
	public static function init() {
		add_action( 'embroidery/tracker/send_event', [ __CLASS__, 'send_tracking_data' ] );
	...
	public static function check_for_settings_optin( $new_value ) {
		$old_value = get_option( 'embroidery_allow_tracking', 'no' );
	...
	public static function send_tracking_data( $override = false ) {
		 ...
		 */
		$override = apply_filters( 'embroidery/tracker/send_override', $override );
		...
		if ( ! $override ) {
			 ...
			 */
			$last_send_interval = apply_filters( 'embroidery/tracker/last_send_interval', $last_send_interval );
		...
		update_option( 'embroidery_tracker_last_send', time() );
		 ...
		 */
		$params = apply_filters( 'embroidery/tracker/send_tracking_data_params', $params );
	...
	public static function is_allow_track() {
		return 'yes' === get_option( 'embroidery_allow_tracking', 'no' );
	}
	...
	public static function handle_tracker_actions() {
		if ( ! isset( $_GET['embroidery_tracker'] ) ) {
			return;
		}

		if ( 'opt_into' === $_GET['embroidery_tracker'] ) {
			check_admin_referer( 'opt_into' );

			update_option( 'embroidery_allow_tracking', 'yes' );
			self::send_tracking_data( true );
		}

		if ( 'opt_out' === $_GET['embroidery_tracker'] ) {
			check_admin_referer( 'opt_out' );

			update_option( 'embroidery_allow_tracking', 'no' );
			update_option( 'embroidery_tracker_notice', '1' );
		}

		wp_redirect( remove_query_arg( 'embroidery_tracker' ) );
		exit;
	}
	 ...
	 * Admin notices.
	 *
	 * Add Embroidery notices to WordPress admin screen to show tracker notice.
	 *
	...
	public static function admin_notices() {
		...
		if ( '1' === get_option( 'embroidery_tracker_notice' ) ) {
			return;
		}
		...
		$optin_url = wp_nonce_url( add_query_arg( 'embroidery_tracker', 'opt_into' ), 'opt_into' );
		$optout_url = wp_nonce_url( add_query_arg( 'embroidery_tracker', 'opt_out' ), 'opt_out' );

		$tracker_description_text = __( 'Love using Embroidery? Become a super contributor by opting in to our anonymous plugin data collection and to our updates. We guarantee no sensitive data is collected.', 'embroidery' );
		 ...
		 */
		$tracker_description_text = apply_filters( 'embroidery/tracker/admin_description_text', $tracker_description_text );
		?>
		<div class="updated">
			<p><?php echo esc_html( $tracker_description_text ); ?> <a href="https://go.embroidery.com/usage-data-tracking/" target="_blank"><?php _e( 'Learn more.', 'embroidery' ); ?></a></p>
			<p><a href="<?php echo $optin_url; ?>" class="button-primary"><?php _e( 'Sure! I\'d love to help', 'embroidery' ); ?></a>&nbsp;<a href="<?php echo $optout_url; ?>" class="button-secondary"><?php _e( 'No thanks', 'embroidery' ); ?></a></p>
		</div>
		<?php
	}
	 ...
	 * Get installed time.
	 *
	 * Retrieve the time when Embroidery was installed.
	 *
	 * @since 1.0.0
	 * @access private
	 * @static
	 *
	 * @return int Unix timestamp when Embroidery was installed.
	 */
	private static function _get_installed_time() {
		$installed_time = get_option( '_embroidery_installed_time' );
		if ( ! $installed_time ) {
			$installed_time = time();
			update_option( '_embroidery_installed_time', $installed_time );
		}
	...
	private static function _get_last_send_time() {
		$last_send_time = get_option( 'embroidery_tracker_last_send', false );
		 ...
		 */
		$last_send_time = apply_filters( 'embroidery/tracker/last_send_time', $last_send_time );
	 ...
	 * Get posts usage.
	 *
	 * Retrieve the number of posts using Embroidery.
	 *
	 * @since 1.0.0
	 * @access private
	 * @static
	 *
	 * @return array The number of posts using Embroidery grouped by post types
	 *               and post status.
	 */
	private static function _get_posts_usage() {
		...
		$results = $wpdb->get_results(
			"SELECT `post_type`, `post_status`, COUNT(`ID`) `hits`
				...
				WHERE `post_type` != 'embroidery_library'
					AND `meta_key` = '_embroidery_edit_mode' AND `meta_value` = 'builder'
	 ...
	 * Get library usage.
	 *
	 * Retrieve the number of Embroidery library items saved.
	 *
	 * @since 1.0.0
	 * @access private
	 * @static
	 *
	 * @return array The number of Embroidery library items grouped by post types
	 *               and meta value.
	 */
	private static function _get_library_usage() {
		...
		$results = $wpdb->get_results(
			"SELECT `meta_value`, COUNT(`ID`) `hits`
				...
				LEFT JOIN {$wpdb->postmeta} `pm` ON(`p`.`ID` = `pm`.`post_id`)
				WHERE `post_type` = 'embroidery_library'
					AND `meta_key` = '_embroidery_template_type'
