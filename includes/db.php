...
/**
 * Embroidery database class.
 *
 * Embroidery database handler class is responsible for comunicating with the
 * DB, save and retrieve Embroidery data and meta data.
 *
 * @since 1.0.0
 */
class DB {
	...
	public function save_editor( $post_id, $data, $status = self::STATUS_PUBLISH ) {
		...
		if ( $save_original ) {
			...
			$is_meta_updated = update_metadata( 'post', $post_id, '_embroidery_data', $json_value );

			/**
			 * Before DB save.
			 *
			 * Fires before Embroidery editor saves data to the database.
			 *
			 * @since 1.0.0
			 *
			 ...
			 */
			do_action( 'embroidery/db/before_save', $status, $is_meta_updated );
			 ...
			 * @param int|bool $is_meta_updated Meta ID if the key didn't exist, true on successful update, false on failure.
			 */
			do_action( 'embroidery/db/before_save', $status, true );
			...
			$autosave_id = wp_create_post_autosave( [
				...
				'post_title' => __( 'Auto Save', 'embroidery' ) . ' ' . date( 'Y-m-d H:i' ),
				'post_content' => $this->get_plain_text_from_data( $editor_data ),
				'post_modified' => current_time( 'mysql' ),
			] );

			if ( $autosave_id ) {
				update_metadata( 'post', $autosave_id, '_embroidery_data', $json_value );
			}
		} // End if().

		update_post_meta( $post_id, '_embroidery_version', self::DB_VERSION );
		 ...
		 * After DB save.
		 *
		 * Fires after Embroidery editor saves data to the database.
		 *
		 * @since 1.0.0
		 *
		 ...
		 */
		do_action( 'embroidery/editor/after_save', $post_id, $editor_data );
	}
	...
	public function get_plain_editor( $post_id, $status = self::STATUS_PUBLISH ) {
		$data = $this->_get_json_meta( $post_id, '_embroidery_data' );

		if ( self::STATUS_DRAFT === $status ) {
			$autosave = $this->get_newer_autosave( $post_id );

			if ( is_object( $autosave ) ) {
				$autosave_data = $this->_get_json_meta( $autosave->ID, '_embroidery_data' );
			}
		}
	 ...
	 *
	 * When editing the with Embroidery the first time, the current page content
	 ...
	 * @param int $post_id Post ID.
	 *
	 * @return array Content in Embroidery format.
	 */
	 ...
	 * Is using Embroidery.
	 *
	 * Set whether the page is using Embroidery or not.
	 *
	 * @since 1.5.0
	 * @access public
	 *
	 * @param int  $post_id      Post ID.
	 * @param bool $is_embroidery Optional. Whether the page is embroidery page.
	 *                           Default is true.
	 */
	public function set_is_embroidery_page( $post_id, $is_embroidery = true ) {
		if ( $is_embroidery ) {
			// Use the string `builder` and not a boolean for rollback compatibility
			update_post_meta( $post_id, '_embroidery_edit_mode', 'builder' );
		} else {
			delete_post_meta( $post_id, '_embroidery_edit_mode' );
		}
	}
	 ...
	 * Get editor data.
	 *
	 * Accepts raw Embroidery data and return parsed data.
	 *
	 * @since 1.0.0
	 * @access private
	 *
	 * @param array $data              Raw Embroidery post data from the database.
	 ...
	 * Iterate data.
	 *
	 * Accept any type of Embroidery data and a callback function. The callback
	 * function runs recursively for each element and his child elements.
	 *
	 * @since 1.0.0
	 * @access public
	 *
	 * @param array    $data_container Any type of embroidery data.
	 ...
	 * @access public
	 */
	public function safe_copy_embroidery_meta( $from_post_id, $to_post_id ) {
		if ( ! Plugin::$instance->db->is_built_with_embroidery( $from_post_id ) ) {
			return;
		}

		// It's from Embroidery, and not from WP-Admin
		if ( did_action( 'embroidery/db/before_save' ) ) {
			return;
		}

		// It's an exited Embroidery auto-save
		if ( get_post_meta( $to_post_id, '_embroidery_data', true ) ) {
			return;
		}

		$this->copy_embroidery_meta( $from_post_id, $to_post_id );
	}

	/**
	 * Copy embroidery meta.
	 *
	 ...
	 */
	public function copy_embroidery_meta( $from_post_id, $to_post_id ) {
		$from_post_meta = get_post_meta( $from_post_id );

		foreach ( $from_post_meta as $meta_key => $values ) {
			// Copy only meta with the `_embroidery` prefix
			if ( 0 === strpos( $meta_key, '_embroidery' ) ) {
				$value = $values[0];

				// The embroidery JSON needs slashes before saving
				if ( '_embroidery_data' === $meta_key ) {
	...
	/**
	 * Is built with Embroidery.
	 *
	 * Check whether the post was built with Embroidery.
	 *
	 * @since 1.0.10
	 * @access public
	 *
	 * @param int $post_id Post ID.
	 *
	 * @return bool Whether the post was built with Embroidery.
	 */
	public function is_built_with_embroidery( $post_id ) {
		return ! ! get_post_meta( $post_id, '_embroidery_edit_mode', true );
	}

	/**
	 * Has Embroidery in post.
	 *
	 * Check whether the post has Embroidery data in the post.
	 *
	 * @since 1.0.10
	 * @access public
	 * @deprecated 1.4.0
	 ...
	 * @return bool Whether the post was built with Embroidery.
	 */
	public function has_embroidery_in_post( $post_id ) {
		return $this->is_built_with_embroidery( $post_id );
	}
