...
use Embroidery\TemplateLibrary\Classes\Import_Images;
...
class Manager {
	...
	*/
	public function on_export_template_error( \WP_Error $error ) {
		_default_wp_die_handler( $error->get_error_message(), 'Embroidery Library' );
	}
	...
	*/
	private function init_ajax_calls() {
		...
		foreach ( $allowed_ajax_requests as $ajax_request ) {
			add_action( 'wp_ajax_embroidery_' . $ajax_request, function() use ( $ajax_request ) {
