<?php
namespace Embroidery\TemplateLibrary;

use Embroidery\DB;
use Embroidery\Core\Settings\Page\Manager as PageSettingsManager;
use Embroidery\Core\Settings\Manager as SettingsManager;
use Embroidery\Core\Settings\Page\Model;
use Embroidery\Editor;
use Embroidery\Plugin;
use Embroidery\Settings;
use Embroidery\User;
...
class Source_Local extends Source_Base {
	...
	const CPT = 'embroidery_library';

	const TAXONOMY_TYPE_SLUG = 'embroidery_library_type';

	const TYPE_META_KEY = '_embroidery_template_type';

	const TEMP_FILES_DIR = 'embroidery/tmp';

	const BULK_EXPORT_ACTION = 'embroidery_export_multiple_templates';
	...
	*/
	public function register_data() {
		$labels = [
			'name' => _x( 'My Library', 'Template Library', 'embroidery' ),
			'singular_name' => _x( 'Template', 'Template Library', 'embroidery' ),
			'add_new' => _x( 'Add New', 'Template Library', 'embroidery' ),
			'add_new_item' => _x( 'Add New Template', 'Template Library', 'embroidery' ),
			'edit_item' => _x( 'Edit Template', 'Template Library', 'embroidery' ),
			'new_item' => _x( 'New Template', 'Template Library', 'embroidery' ),
			'all_items' => _x( 'All Templates', 'Template Library', 'embroidery' ),
			'view_item' => _x( 'View Template', 'Template Library', 'embroidery' ),
			'search_items' => _x( 'Search Template', 'Template Library', 'embroidery' ),
			'not_found' => _x( 'No Templates found', 'Template Library', 'embroidery' ),
			'not_found_in_trash' => _x( 'No Templates found in Trash', 'Template Library', 'embroidery' ),
			'parent_item_colon' => '',
			'menu_name' => _x( 'My Library', 'Template Library', 'embroidery' ),
		];
		...
		$args = [
			...
			'hierarchical' => false,
			'supports' => [ 'title', 'thumbnail', 'author', 'embroidery' ],
		];

		/**
		 * Register template library post type args.
		 *
		 * Filters the post type arguments when registering embroidery template library post type.
		 *
		 * @since 1.0.0
		 *
		 * @param array $args Arguments for registering a post type.
		 */
		$args = apply_filters( 'embroidery/template_library/sources/local/register_post_type_args', $args );
		...
		$args = [
			...
			'rewrite' => false,
			'public' => false,
			'label' => _x( 'Type', 'Template Library', 'embroidery' ),
		];

		/**
		 * Register template library taxonomy args.
		 *
		 * Filters the taxonomy arguments when registering embroidery template library taxonomy.
		 *
		 * @since 1.0.0
		 *
		 * @param array $args Arguments for registering a taxonomy.
		 */
		$args = apply_filters( 'embroidery/template_library/sources/local/register_taxonomy_args', $args );
	...
	*/
	public function register_admin_menu() {
		if ( current_user_can( 'manage_options' ) ) {
			add_submenu_page(
				Settings::PAGE_ID,
				_x( 'My Library', 'Template Library', 'embroidery' ),
				_x( 'My Library', 'Template Library', 'embroidery' ),
	...
	*/
	public function save_item( $template_data ) {
		...
		if ( ! current_user_can( $this->post_type_object->cap->edit_posts ) ) {
			return new \WP_Error( 'save_error', __( 'Access Denied.', 'embroidery' ) );
		}

		$template_id = wp_insert_post( [
			'post_title' => ! empty( $template_data['title'] ) ? $template_data['title'] : __( '(no title)', 'embroidery' ),
		...
		Plugin::$instance->db->set_is_embroidery_page( $template_id );
		 ...
		 *
		 * Fires after Embroidery template library was saved.
		 *
		 * @since 1.0.1
		 *
		 ...
		 */
		do_action( 'embroidery/template-library/after_save_template', $template_id, $template_data );

		/**
		 * After template library update.
		 *
		 * Fires after Embroidery template library was updated.
		 *
		 * @since 1.0.1
		 *
		 * @param int   $template_id   The ID of the template.
		 * @param array $template_data The template data.
		 */
		do_action( 'embroidery/template-library/after_update_template', $template_id, $template_data );
	...
	*/
	public function update_item( $new_data ) {
		 ...
		 */
		do_action( 'embroidery/template-library/after_update_template', $new_data['id'], $new_data );
	...
	public function get_item( $template_id ) {
		 ...
		 *
		 * Filters the embroidery template data when loading template library item.
		 *
		 * @since 1.0.0
		 *
		 * @param array $data Arguments for registering a taxonomy.
		 */
		$data = apply_filters( 'embroidery/template-library/get_template', $data );
	...
	*/
	public function delete_template( $template_id ) {
		if ( ! current_user_can( $this->post_type_object->cap->delete_post, $template_id ) ) {
			return new \WP_Error( 'template_error', __( 'Access Denied.', 'embroidery' ) );
		}
	...
	*/
	public function export_multiple_templates( array $template_ids ) {
		 ...
		 */
		$zip_archive_filename = 'embroidery-templates-' . date( 'Y-m-d' ) . '.zip';
	...
	public function post_row_actions( $actions, \WP_Post $post ) {
		if ( self::is_base_templates_screen() ) {
			if ( $this->is_template_supports_export( $post->ID ) ) {
				$actions['export-template'] = sprintf( '<a href="%s">%s</a>', $this->_get_export_link( $post->ID ), __( 'Export Template', 'embroidery' ) );
			}
	...
	*/
	public function admin_import_template_form() {
		...
		?>
		<div id="embroidery-hidden-area">
			<a id="embroidery-import-template-trigger" class="page-title-action"><?php esc_attr_e( 'Import Templates', 'embroidery' ); ?></a>
			<div id="embroidery-import-template-area">
				<div id="embroidery-import-template-title"><?php esc_attr_e( 'Choose an Embroidery template JSON file or a .zip archive of Embroidery templates, and add them to the list of templates available in your library.', 'embroidery' ); ?></div>
				<form id="embroidery-import-template-form" method="post" action="<?php echo admin_url( 'admin-ajax.php' ); ?>" enctype="multipart/form-data">
					<input type="hidden" name="action" value="embroidery_import_template">
					<input type="hidden" name="_nonce" value="<?php echo Plugin::$instance->editor->create_nonce( self::CPT ); ?>">
					<fieldset id="embroidery-import-template-form-inputs">
						...
						<input type="submit" class="button" value="<?php esc_attr_e( 'Import Now', 'embroidery' ); ?>">
					</fieldset>
				</form>
			</div>
		</div>
	...
	*/
	public function is_template_supports_export( $template_id ) {
		 ...
		 */
		$export_support = apply_filters( 'embroidery/template_library/is_template_supports_export', $export_support, $template_id );
	 ...
	 * @access public
	 */
	public function remove_embroidery_post_state_from_library( $post_states, $post ) {
		if ( self::CPT === $post->post_type && isset( $post_states['embroidery'] ) ) {
			unset( $post_states['embroidery'] );
		}
		return $post_states;
	}
	...
	*/
	private function _get_export_link( $template_id ) {
		return add_query_arg(
			[
				'action' => 'embroidery_export_template',
				'source' => $this->get_id(),
	...
	*/
	public function admin_add_bulk_export_action( $actions ) {
		$actions[ self::BULK_EXPORT_ACTION ] = __( 'Export', 'embroidery' );
	...
	*/
	private function prepare_template_export( $template_id ) {
		...
		return [
			'name' => 'embroidery-' . $template_id . '-' . date( 'Y-m-d' ) . '.json',
	...
	*/
	private function _add_actions() {
		if ( is_admin() ) {
			...
			add_filter( 'display_post_states', [ $this, 'remove_embroidery_post_state_from_library' ], 11, 2 );

			// template library bulk actions.
			add_filter( 'bulk_actions-edit-embroidery_library', [ $this, 'admin_add_bulk_export_action' ] );
			add_filter( 'handle_bulk_actions-edit-embroidery_library', [ $this, 'admin_export_multiple_templates' ], 10, 3 );
