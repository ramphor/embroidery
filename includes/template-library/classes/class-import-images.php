<?php
namespace Embroidery\TemplateLibrary\Classes;
		...
		$post_id = $wpdb->get_var(
			$wpdb->prepare(
				'SELECT `post_id` FROM `' . $wpdb->postmeta . '`
					WHERE `meta_key` = \'_embroidery_source_image_hash\'
						AND `meta_value` = %s
				;',
	...
	*/
	public function import( $attachment ) {
		...
		if ( $info ) {
			...
			return $attachment;
			// return new \WP_Error( 'attachment_processing_error', __( 'Invalid file type', 'embroidery' ) );
		}
		...
		update_post_meta( $post_id, '_embroidery_source_image_hash', $this->_get_hash_image( $attachment['url'] ) );
