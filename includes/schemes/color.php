...
/**
 * Embroidery color scheme.
 *
 * Embroidery color scheme class is responsible for initializing a scheme for
 * colors.
 *
 * @since 1.0.0
 */
class Scheme_Color extends Scheme_Base {
	...
	public function get_disabled_title() {
		return __( 'Color Palettes', 'embroidery' );
	}
	...
	public function get_scheme_titles() {
		return [
			self::COLOR_1 => __( 'Primary', 'embroidery' ),
			self::COLOR_2 => __( 'Secondary', 'embroidery' ),
			self::COLOR_3 => __( 'Text', 'embroidery' ),
			self::COLOR_4 => __( 'Accent', 'embroidery' ),
		];
	}
	...
	public function print_template_content() {
		?>
		<div class="embroidery-panel-scheme-content embroidery-panel-box">
			<div class="embroidery-panel-heading">
				<div class="embroidery-panel-heading-title"><?php echo $this->_get_current_scheme_title(); ?></div>
			</div>
			<?php
			...
			if ( $description ) :
				?>
				<div class="embroidery-panel-scheme-description embroidery-descriptor"><?php echo $description; ?></div>
			<?php endif; ?>
			<div class="embroidery-panel-scheme-items embroidery-panel-box-content"></div>
		</div>
		<div class="embroidery-panel-scheme-colors-more-palettes embroidery-panel-box">
			<div class="embroidery-panel-heading">
				<div class="embroidery-panel-heading-title"><?php _e( 'More Palettes', 'embroidery' ); ?></div>
			</div>
			<div class="embroidery-panel-box-content">
				<?php foreach ( $this->_get_system_schemes_to_print() as $scheme_name => $scheme ) : ?>
					<div class="embroidery-panel-scheme-color-system-scheme" data-scheme-name="<?php echo $scheme_name; ?>">
						<div class="embroidery-panel-scheme-color-system-items">
							<?php foreach ( $scheme['items'] as $color_value ) : ?>
								<div class="embroidery-panel-scheme-color-system-item" style="background-color: <?php echo esc_attr( $color_value ); ?>;"></div>
							<?php endforeach; ?>
						</div>
						<div class="embroidery-title"><?php echo $scheme['title']; ?></div>
					</div>
				<?php endforeach; ?>
			</div>
		</div>
	...
	protected function _get_current_scheme_title() {
		return __( 'Color Palette', 'embroidery' );
	}
}
