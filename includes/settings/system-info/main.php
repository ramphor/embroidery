<?php
namespace Embroidery\System_Info;

use Embroidery\System_Info\Classes\Abstracts\Base_Reporter;
use Embroidery\System_Info\Helpers\Model_Helper;
...
class Main {
	...
	*/
	private function add_actions() {
		...
		add_action( 'wp_ajax_embroidery_system_info_download_file', [ $this, 'download_file' ] );
	}
	...
	*/
	public function display_page() {
		...
		?>
		<div id="embroidery-system-info">
			<h3><?php _e( 'System Info', 'embroidery' ); ?></h3>
			<div><?php $this->print_report( $reports, 'html' ); ?></div>
			<h3><?php _e( 'Copy & Paste Info', 'embroidery' ); ?></h3>
			<div id="embroidery-system-info-raw">
				<label id="embroidery-system-info-raw-code-label" for="embroidery-system-info-raw-code"><?php _e( 'You can copy the below info as simple text with Ctrl+C / Ctrl+V:', 'embroidery' ); ?></label>
				<textarea id="embroidery-system-info-raw-code" readonly>
					<?php
				...
				<script>
					var textarea = document.getElementById( 'embroidery-system-info-raw-code' );
			...
			<form action="<?php echo admin_url( 'admin-ajax.php' ); ?>" method="post">
				<input type="hidden" name="action" value="embroidery_system_info_download_file">
				<input type="submit" class="button button-primary" value="<?php _e( 'Download System Info', 'embroidery' ); ?>">
			</form>
		</div>
		<?php
	}
	...
	*/
	public function download_file() {
		if ( ! current_user_can( $this->capability ) ) {
			wp_die( __( 'You don\'t have a permission to download this file', 'embroidery' ) );
		}
	...
	*/
	public function register_menu() {
		$system_info_text = __( 'System Info', 'embroidery' );
	...
	*/
	protected function get_default_settings() {
		...
		$base_lib_dir = EMBROIDERY_PATH . 'includes/settings/system-info/';
