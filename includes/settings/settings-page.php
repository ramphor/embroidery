	...
	public final function register_settings_fields() {
		...
		foreach ( $tabs as $tab_id => $tab ) {
			...
			foreach ( $tab['sections'] as $section_id => $section ) {
				$full_section_id = 'embroidery_' . $section_id . '_section';
				...
				foreach ( $section['fields'] as $field_id => $field ) {
					$full_field_id = ! empty( $field['full_field_id'] ) ? $field['full_field_id'] : 'embroidery_' . $field_id;
	...
	*/
	public function display_settings_page() {
		...
		<div class="wrap">
			...
			<div id="embroidery-settings-tabs-wrapper" class="nav-tab-wrapper">
				<?php
				foreach ( $tabs as $tab_id => $tab ) {
					...
					echo "<a id='embroidery-settings-tab-{$tab_id}' class='nav-tab{$active_class}' href='#tab-{$tab_id}'>{$tab['label']}</a>";
				}
				?>
			</div>
			<form id="embroidery-settings-form" method="post" action="options.php">
				<?php
				...
				foreach ( $tabs as $tab_id => $tab ) {
					...
					if ( 'general' === $tab_id ) {
						$active_class = ' embroidery-active';
					}

					echo "<div id='tab-{$tab_id}' class='embroidery-settings-form-page{$active_class}'>";
	...
	*/
	private function ensure_tabs() {
		if ( null === $this->tabs ) {
			 ...
			 *
			 * Fires after the settings are created in Embroidery admin page.
			 *
			 ...
			 */
			do_action( "embroidery/admin/after_create_settings/{$page_id}", $this );
		}
	}
}
