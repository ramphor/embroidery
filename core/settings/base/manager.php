<?php
namespace Embroidery\Core\Settings\Base;

use Embroidery\CSS_File;
...
abstract class Manager {
	...
	public function __construct() {
		if ( Utils::is_ajax() ) {
			add_action( 'wp_ajax_embroidery_save_' . $this->get_name() . '_settings', [ $this, 'ajax_save_settings' ] );
		}

		add_action( 'embroidery/init', [ $this, 'on_embroidery_init' ] );

		add_action( 'embroidery/' . $this->get_css_file_name() . '-css-file/parse', [ $this, 'add_settings_css_rules' ] );
	}
	...
	final public function ajax_save_settings() {
		 ...
		 */
		$success_response_data = apply_filters( "embroidery/{$settings_name}/settings/success_response_data", $success_response_data, $id, $data );
	 ...
	 * @access public
	 */
	public function on_embroidery_init() {
	...
	protected function print_editor_template_content( $name ) {
		...
		<div class="embroidery-panel-navigation">
			<# _.each( embroidery.config.settings.<?php echo $name; ?>.tabs, function( tabTitle, tabSlug ) { #>
				<div class="embroidery-panel-navigation-tab embroidery-tab-control-{{ tabSlug }}" data-tab="{{ tabSlug }}">
					<a href="#">{{{ tabTitle }}}</a>
				</div>
				<# } ); #>
		</div>
		<div id="embroidery-panel-<?php echo $name; ?>-settings-controls"></div>
		<?php
	}
	...
	private function get_editor_template() {
		...
		ob_start();
		?>
		<script type="text/template" id="tmpl-embroidery-panel-<?php echo $name; ?>-settings">
